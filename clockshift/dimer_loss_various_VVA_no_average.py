...
for i, file in enumerate(files):
	...
	run.group_by_median('VVA')
	for VVA_val in drop_VVAs:
		run.med_data = run.med_data.drop(run.med_data.loc[(run.med_data.VVA == VVA_val)].index)
	
		
	# correct ratio average and it's unceratinty
# 	run.med_data['ratio'] = run.med_data['c5_loss']/run.med_data['c9_loss']
# 	run.med_data['em_ratio'] = run.med_data['ratio']*np.sqrt((run.med_data['em_c5_loss']/run.med_data['c5_loss'])**2 +\
#  	 									  (run.med_data['em_c9_loss']/run.med_data['c9_loss'])**2)
		
# 	run.med_data['fraction'] = run.med_data['c5_loss']/(run.med_data['c9_loss']+run.med_data['c5_loss'])
# 	run.med_data['em_fraction'] = ?
	
	dfs.append(run.med_data)
	# propagate bg uncertainty, this is not a statistical fluctuation though...
	if bg_uncertainty_propagate == True:
		for spin in spins:
			run.med_data['em_'+spin+'_loss'] = np.sqrt(run.med_data['em_'+spin+'_loss']**2 \
												+ run.med_data[spin+'_bg_std'].unique()[0]**2)
	
	# plot
	x = run.med_data[xname]	
	
	for spin, sty in zip(spins, styles):
		# counts
		y = run.med_data[spin]
		ylower = y-run.med_data['lower_'+spin]
		yupper =run.med_data['upper_'+spin]-y
		axs[0].errorbar(x, y, (np.abs(ylower), np.abs(yupper)),
					 label=spin_map(spin), **sty)
		bg = run.data[spin+'_bg'].unique()*np.ones(len(x))
		axs[0].plot(x, bg, '--')
		# loss
		run.med_data[spin+'_loss']
		axs[1].errorbar(x, y, (np.abs(ylower), np.abs(yupper)),
					 label=spin_map(spin), **sty)
		# widths
		y = run.med_data[spin+'_s']
		axs[2].errorbar(x, y, (np.abs(y-run.med_data['lower_'+spin+'_s']), np.abs(run.med_data['upper_'+spin+'_s']-y)), 
					 label=spin_map(spin), **sty)
		
		# quantiles
		qsize = yupper-ylower
		axs[6].plot(x, qsize, label=spin_map(spin), **sty)
	...
	y = run.med_data['c_'+rof]
	axs[3].errorbar(x, y, (np.abs(y-run.med_data['lower_'+'c_'+rof]), 
					np.abs(run.med_data['upper_'+'c_'+rof]-y)),
					label=label, **sty)
	# loss ratio/fraction
	y = run.med_data[rof]
	axs[4].errorbar(x, run.med_data[rof], (np.abs(y-run.med_data['lower_'+rof]), 
					np.abs(run.med_data['upper_'+rof]-y)),
					label=label, **sty)
	# width ratio/fraction
	y = run.med_data['s_'+rof]
	axs[5].errorbar(x, y, (np.abs(y-run.med_data['lower_'+'s_'+rof]), 
					np.abs(run.med_data['upper_'+'s_'+rof]-y)),
					label=label, **sty)
...
ax.set(xlabel="Magnetic Field (G)", ylabel="b/a Loss "+rof)
