...
figsize = (4.8, 2.4)  # One-column PRL figure size in inches
	
paper_settings = {
				'font.size': 10,          # Base font size
				'axes.labelsize': 12,       # Axis label font size
				'axes.titlesize': 8,       # Title font size (if used)
				'xtick.labelsize': 10,      # Tick label font size (x-axis)
				'ytick.labelsize': 10,      # Tick label font size (y-axis)
				'legend.fontsize': 10,      # Legend font size
				'figure.figsize': figsize, 
				'figure.dpi': 200,        # Publication-ready resolution
				'lines.linewidth': 1,      # Thinner lines for compactness
				"lines.linestyle":'',
				'lines.markeredgewidth': 1.2, # Thinner marker edges
...
mycolors = ['#ff7f0e', "#d353ada8", "#17cfc9"]
mystyles = generate_plt_styles(mycolors, ts=0.5)
...
for i, file in enumerate(files):
	...
	for i, string in enumerate(transfer_loss_strs):
		...
		ylabel = r'$4\alpha/\Omega_R^2t_\mathrm{rf}^2$'
		xlabel = r'$\Omega_R^2t_\mathrm{rf}^2$'
		title = file
...
for i, df in enumerate(df_list):
	ax = axs[0]
	ax.set(xlabel=r'$\Omega_R^2 t_\mathrm{rf}^2$', ylabel=r'$\alpha$', ylim=[-0.05, 1.20],
		xlim=[0, 15])
	...
	ax = axs[1]
	ax.set(xlabel=r'$\Omega_R^2 t_\mathrm{rf}^2$', ylabel=r'$\alpha$', 
		ylim=[-0.05, 0.5],
		xlim=[0, 3])
	ax.plot(df['OmegaR2t2'], df['alpha_transfer'], label=labels[i] + ', transfer', **mystyles[i])
	ax.plot(df['OmegaR2t2'], df['alpha_loss'], label=labels[i]+', loss', mfc='white')
	...
	ax = axs[2]
	ax.set(xlabel=r'$\Omega_R t_\mathrm{rf}$', ylabel=r'$\alpha$', 
		xlim = [0,8],
		ylim=[-0.05, 1])
...
axs[0].set(xlabel=r'$\Omega_R^2 t_\mathrm{rf}^2$', ylabel=r'Transfer $\alpha$', ylim=[-0.03, 1],
		xlim=[-0.02, 20]
		)
# repeat but zoomed in
axs[2].set(xlabel=r'$\Omega_R^2 t_\mathrm{rf}^2$', ylabel=r'Transfer $\alpha$', ylim=[-0.03, 0.12],
	xlim=[-0.02, 0.6])
# plot alpha_loss vs Rabi^2time^2
axs[1].set(xlabel=r'$\Omega_R^2 t_\mathrm{rf}^2$', ylabel=r'Loss $\alpha$', ylim=[-0.03, 1],
		xlim=[-0.02, 20]
		)
# repeat but zoomed in
axs[3].set(xlabel=r'$\Omega_R^2 t_\mathrm{rf}^2$', ylabel=r'Loss $\alpha$', ylim=[-0.03, 0.12],
	xlim=[-0.02, 0.6])
...
for i, df in enumerate(df_list):
	...
	yname = 'alpha_transfer'
	dfgrp = df.groupby(xname).agg(['mean', 'std','sem']).reset_index()
	ax.errorbar(dfgrp[xname], dfgrp[yname]['mean'], yerr=dfgrp[yname][err_type], 
			 label=label, **mystyles[i])
...
fig.tight_layout()
plt.savefig(proj_path + '\manuscript\manuscript_figures\SM_Zfactors.pdf', dpi=300)
plt.show()
