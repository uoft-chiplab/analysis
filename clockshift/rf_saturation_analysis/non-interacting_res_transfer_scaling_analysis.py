...
if __name__ == '__main__':
	...
	files = [
		  "2025-02-27_D_e_detuning=-4.dat",
		  "2025-02-27_D_e_detuning=-2.dat",
		  "2025-02-27_D_e_detuning=-1.5.dat",
		  "2025-02-27_D_e_detuning=-1.dat",
		  "2025-02-27_D_e_detuning=-0.5.dat",
		  "2025-02-27_D_e_detuning=0.dat",
		  "2025-02-27_D_e_detuning=1.dat",
		  "2025-02-27_D_e_detuning=2.dat",
		  "2025-02-27_D_e_detuning=4.dat",
		  ]
	...
	plt.rcParams.update({"figure.figsize": [6.8,4],
						 "font.size": 14,
	...
	for i in range(len(detunings)):
		...
		# VVA greater than some cutoff determined from first pass fit
		cutoff_df = run.avg_data.loc[(run.avg_data.VVA > cutoff)]
		...
		# plot cwc 
		ax = axs[4]
		ax.set(xlabel=xlabel, ylabel='Nb cwc')
		y = run.avg_data['c9cwc']/run.avg_data['c9']
		yerr = run.avg_data['em_c9cwc']/run.avg_data['c9']
		...
		# plot cwc 
		ax = axs[5]
		ax.set(xlabel=xlabel, ylabel='Nb cwc')#, ylim=[10,38])
		y = run.avg_data['c9cwc']/run.avg_data['c9']
		yerr = (run.avg_data['em_c9cwc']/run.avg_data['c9'])
		ax.errorbar(x, y, yerr=yerr, **sty, label=label)
...
# 	axs[j].set_ylim(0,2)
	...
	print("SW from transfer is {:.2f}".format(SW))
	print("SW from loss is {:.2f}".format(SW_l))
	
	if integral_error == True:
