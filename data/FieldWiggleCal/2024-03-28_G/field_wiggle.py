...
wiggle_freq = 50.0 # kHz
wiggle_amp = 0.9 # Vpp
pulse_length = 2 # us
note = ''

# parameters relevent for analysis
regex = re.compile(run + '_e_windowsamples=(\d+).dat') # this string probably has to be adjusted for each data set
x_name = "freq"
y_name = "fraction95"
fit_func = Sinc2
num = 500
...
for file in os.listdir(data_folder):
	...
	time = delaytime + pulse_length/2 # choose middle of pulse
...
# fit B vs. time
func=FixedSinkHz
param_bounds = [[-np.inf, 0, -np.inf],[np.inf, 2*np.pi, np.inf]]
popt, pcov = curve_fit(func, data_df['time'], data_df['B'], sigma=data_df['e_B'], bounds=param_bounds)
