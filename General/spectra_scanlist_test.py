...
xvals = run.avg_data[run.avg_data['transfer'] < cutoff]['OmegaR2']
...
xlist = np.linspace(min(xvals),0.07,1000)
...
for ax, ynameset, ylabel in zip(axs.flatten(), ynamesets, ylabels):
	...
	if i == 4: 
		ax.set(xlim=[min(xlist), 0.04], ylim=[-0.02, cutoff])
		ax.plot(xlist,func(xlist,*poptt),linestyle='-', marker='')
		ax.plot(xlist, func(xlist, *poptl),linestyle='--',marker='')
...
# func for contact (transfer) vs. T
# func for transfer vs. VVA
flist = np.linspace(0, 500, 100) # kHz
fscale = flist**(-3/2) / 100**(-3/2)
fig, ax = plt.subplots()
ax.plot(flist, fscale)

def fscale(f):
	fs = f**(-3/2) / 100**(-3/2)
	return fs

df = pd.read_excel('E:\\Analysis Scripts\\analysis\\clockshift\\sumrule_analysis_results.xlsx')
poptC, pcovC = curve_fit(Linear, df['ToTF'], df['C_median'])
fig, ax = plt.subplots()
ax.plot(df['ToTF'], Linear(df['ToTF'], *poptC) / Linear(ToTF, *poptC))

def Tscale(T):
 	ts = Linear(T, *poptC) / Linear(ToTF, *poptC)
 	return ts

fig, ax = plt.subplots()
interp = interp1d(run.data.OmegaR2.unique(),run.data.VVA.unique(), bounds_error=False,fill_value='extrapolate')
ax.plot(run.data.OmegaR2,run.data.VVA)
ax.plot(np.sort(run.data.OmegaR2.unique()), interp(np.sort(run.data.OmegaR2.unique())), '--')

def VVApert(f, T, transfer=0.1):
	fs = fscale(f)
	Ts = Tscale(T)
	corr = 1/(Ts*fs)
	restransfer = 0.015
	OmegaR2set = np.where(f > 20,(transfer - poptl[1])/poptl[0], (restransfer- poptl[1])/poptl[0] )
	OmegaR2new = corr*OmegaR2set
	VVA = interp(OmegaR2new)
	VVA = np.clip(VVA, 0, 10)
# 	print(OmegaR2new))
	
	return VVA

flist = np.linspace(0, 500, 200)
Tlist = np.array([0.3, 0.4, 0.5, 0.6, 0.7])
colors = ['b','g','r','c','m']
fig, ax=plt.subplots()
for i, T in enumerate(Tlist):
	ax.plot(flist, VVApert(flist, T), mfc=colors[i],label=f'T={T}')
ax.set(xlabel = 'detuning [kHz]', ylabel='VVA')
ax.legend()
	
num = 10
detlist1 = np.linspace(25,250,num)
detlist2 = np.linspace(300,500,3)
# detlist3 = np.linspace(0,24,7)
detlist = np.concatenate((detlist1, detlist2))
T = 0.42
VVAlist = VVApert(detlist, T, transfer=0.16)

bgpts = -1000*np.ones(int(num/5))
detlist = np.concatenate((detlist, bgpts))
detlist = detlist/1e3 # put into MHz so it doesn't end the scan when you have -1000
VVAlist = np.concatenate((VVAlist, 10*np.ones(int(num/5))))
frequency = 47.2227 + detlist

np.savetxt('D:\\LocalCode\\scanlists\\0p42ToTF_spectra.txt', 
 		   np.column_stack((detlist, frequency, VVAlist)), delimiter='\t')
