...
meta_df = meta_df.loc[meta_df['filename'] == '2024-06-18_D_e.dat']
...
# skipset=np.array([0.040,0.060,0.080,0.100]) * \
#       1000 + meta_df.pulselength.values[0]/2
# df = df[~df.time.isin(skipset)]
df = df[df.time < 200]

# skipset=np.array([0.26,0.28]) * \
#       1000 + meta_df.pulselength.values[0]/2
# df = df[~df.time.isin(skipset)]
...
Anormfix_perr = np.sqrt(np.diag(Anormfix_popt))
...
ps_Anormfix = pAnormfix - pdB
e_psAnormfix = np.sqrt(pAnormfix_err**2 + pdB_err**2)
...
#residuals
yyAres = df.A.unique() - sinefit_func(times, *A_popt)
yyAnormres = df[Anorm_name].unique() - sinefit_func(times, *Anorm_popt) 
fig, ax = plt.subplots()
ax.plot(times, yyAres, 'ro')
