...
import pandas as pd
import numpy as np
from fit_functions import Gaussian, Sinc2, Dimerlineshape, Lorentzian, DimerlineshapeZero
...
# 	bg = subrun.data[(subrun.data[x_name] <= 43.05) | (subrun.data[x_name] >= 43.35)]
# 	ynorm = np.mean(bg[y_name])
	ynorm=max(subrun.data[y_name])
	subrun.data[y_name] = -(subrun.data[y_name] - ynorm) # positive ''transfer''
	...
	[subrun.data['A'], subrun.data['f0'], subrun.data['sigma']] = \
        [*subrun.popt]

	[subrun.data['e_A'], subrun.data['e_f0'], subrun.data['e_sigma']] = \
        [*subrun.perr]
